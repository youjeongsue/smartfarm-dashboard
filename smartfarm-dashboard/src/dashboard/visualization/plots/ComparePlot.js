...
const ComparePlot = ({ inputsForPlot }) => {

    const { data, data2, code, startDate, endDate } = inputsForPlot;
    ...
    const plotGraph2 = (reducedData, reducedData2) => {
        const config = {
            layout: {
                margin: {
                    l:30, r:30, t:10, b:30
            ...
            return (
                <Plot className="detail-plot"
                    data={[
                        ...
                        {
                            x: reducedData2['measDate'],
                            y: reducedData2['senVal'],
                            type: 'scatter',
                            mode: 'lines+markers'
                        }
                    ]}
    ...
    return (
        <div>
            {plotGraph2(reduce(filterByDate(filterByCode(data, code))), reduce(filterByDate(filterByCode(data2, code))))}
        </div>
    );
}
